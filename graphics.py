...
# Version 5.0.1 1/25/2021 (updated by Titus H. Klinge)
#     * added a _str_ method to the Image class to make it easy
#       to display in Thonny
#     * added a check_color function that helps ensure that colors are
#       converted correctly without needing to convert back and forth
#       between [r, g, b] and color_rgb(r, g, b) and verify that
#       all components are between 0 and 255.
...
class Image(GraphicsObject):
    ...
    def __str__(self):
        """Displays the image in the Thonny shell

        Author:
            Titus H. Klinge"""
        try:
            # Creates a temporary PNG file location
            fd, path = tempfile.mkstemp(suffix=".png")

            # Exports the image as a PNG to the temp file location
            self.save(path)
            
            # Reads the PNG in a base64-encoded string and then
            # prints it in a format that Thonny supports
            with open(path, "rb") as f: 
                data = base64.b64encode(f.read())
                return "data:image/png;base64," + data.decode()

            # Make sure to remove the temporary file when
            # we are done with everything
            os.remove(path)
        
        # If something goes wrong, just output the usual
...
def check_color(color):
    """Ensures that a color is in a format compatible with tkinter.
    Supports colors in the form of [r,g,b], (r,g,b), and "black"""
...
def color_rgb(r,g,b):
    ...
    f = lambda c: min(max(0, round(c)), 255)
